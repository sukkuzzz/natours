...
I built a full-stack Tour Management System using Node.js, Express.js, MongoDB, and Pug. The application allows users to explore, book, and manage tours efficiently. The backend handles tour data and user interactions through secure and well-structured APIs, while the frontend, built with Pug, provides a dynamic and user-friendly interface. The home page displays ongoing tours with real-time updates and a section featuring reviews and highlights for past tours. This project showcases robust backend development and a visually appealing and functional frontend.
...
Users can update their password securely by providing the current password for validation.
...
User Schema: Includes fields for name, email, password, role (admin or user), and bookings (referencing tour data).
Tour Schema: Linked with bookings, containing name, location, dates, and associated user IDs.
...
I incorporated Mapbox into the Tour Management System to provide an interactive and visually appealing map for each tour's location. Users can view the tour's exact starting point and important landmarks on a dynamic map, enhancing the overall experience and helping them plan their trips better.

The MongoDB schema was designed to integrate seamlessly with Mapbox by including geospatial data fields, such as:

Location Coordinates: Stored as GeoJSON objects (type and coordinates) representing the tour's exact location.
...
Users can book tours directly through the platform using Stripe's payment gateway, ensuring secure transactions with multiple payment options. After selecting a tour, users are guided to a checkout page where they can enter payment details and confirm their booking.

To support this feature, the backend was designed to handle:

Booking Management: Storing booking details in MongoDB, including user information, selected tour, payment status, and transaction ID.
